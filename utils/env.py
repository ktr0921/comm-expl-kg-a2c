...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #kelvin
GraphInfo = collections.namedtuple('GraphInfo', 'objs, ob_rep, act_rep, graph_state, graph_state_rep, admissible_actions, admissible_actions_rep, observation_look')
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
class KGA2CEnv:
    ...
    def __init__(self, rom_path, seed, spm_model, tsv_file, step_limit=None, stuck_steps=10, gat=True,
                 redis_port=6970, openie_port=9010):
        random.seed(seed)
        np.random.seed(seed)
    ...
    def create(self):
        ...
        self.conn_valid = redis.Redis(host='localhost', port=self.redis_port, db=0)
        self.conn_openie = redis.Redis(host='localhost', port=self.redis_port, db=1)
    ...
    def _build_graph_rep(self, action, ob_r):
        ...
        interactive_object_token = [o[0] for o in self.env.identify_interactive_objects(ob_r)]
        interactive_object_token.append('all')
        admissible_actions = self._get_admissible_actions(interactive_object_token)
        ...
        try:  # Gather additional information about the new state
            ...
            observation_look = self.env.step('look')[0]
            self.env.load_str(save_str)
            observation_inventory = self.env.step('inventory')[0]
            self.env.load_str(save_str)
        except RuntimeError:
            print('RuntimeError: {}, Done: {}, Info: {}'.format(clean_obs(ob_r), done, info))
            observation_look = observation_inventory = ''
        all_observation_rep = self.state_rep.get_obs_rep(observation_look, observation_inventory, ob_r, action)
        cleaned_obs = clean_obs(observation_look + ' ' + ob_r)
        cleaned_obs = 'None.' if cleaned_obs == '' else cleaned_obs
        openie_cache = self.conn_openie.get(cleaned_obs)
        if openie_cache is None:
            rules, tocache = self.state_rep.step(cleaned_obs, observation_inventory, interactive_object_token, action, cache=None, gat=self.gat)
            self.conn_openie.set(cleaned_obs, str(tocache))
        else:
            openie_cache = eval(openie_cache.decode('cp1252'))
            rules, _ = self.state_rep.step(cleaned_obs, observation_inventory, interactive_object_token, action, cache=openie_cache, gat=self.gat)
        ...
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #kelvin
        return GraphInfo(
            interactive_object_token, all_observation_rep, action_rep, graph_state, graph_state_rep,
            admissible_actions, admissible_actions_rep, observation_look
        )
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        # return GraphInfo(
        #     interactive_object_token, all_observation_rep, action_rep, graph_state, graph_state_rep,
    ...
    def reset(self):
        self.state_rep = StateAction(self.spm_model, self.vocab, self.vocab_rev, self.tsv_file, self.max_word_len, openie_port=self.openie_port)
        self.stuck_steps = 0
        self.valid_steps = 0
        self.episode_steps = 0
